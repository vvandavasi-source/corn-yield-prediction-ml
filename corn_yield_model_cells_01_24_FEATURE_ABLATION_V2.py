...
# ============================================================
# CELL 19 — OPTIONAL STATIC SOILGRIDS SOC / CEC DISCOVERY
#
# IMPORTANT:
#   The current production model through Cell 18 DOES use:
#       • PRISM weather (15 compact raw weather features)
#       • 7 strict historical PRISM anomaly features
#       • SoilGrids AWC100
#       • 3 AWC × weather-stress interactions
#       • hist_5yr as an explicit predictor
#       • year as an explicit predictor
#
#   It DOES NOT currently use SOC or CEC.
#
# This cell discovers SOC/CEC columns from staged SoilGrids CSVs so we
# can test whether adding them improves held-out-year performance.
# ============================================================

print()
print("=" * 105)
print("CELL 19 — DISCOVER STATIC SOILGRIDS SOC / CEC FEATURES")
print("=" * 105)


def _soil_file_score(path):
    lower = os.path.basename(path).lower()

    score = 0

    if "soilgrids" in lower:
        score += 4
...
soil_soc_cec_candidates = [
    ...
    if (
        "soil" in os.path.basename(f).lower()
        and
        (
            "soc" in os.path.basename(f).lower()
            or
            "cec" in os.path.basename(f).lower()
            or
            "soilgrids" in os.path.basename(f).lower()
        )
    )
...
soil_soc_cec_candidates = sorted(
...
CEC_FEATURES = []
SOC_CEC_FEATURES = []
SOIL_SOC_CEC_FILE = None


for candidate in soil_soc_cec_candidates:
    ...
    try:
        ...
        # ----------------------------------------------------
        # Discover numeric SOC and CEC columns dynamically.
        # This avoids hard-coding a depth/profile naming scheme.
        # ----------------------------------------------------
        ...
        for col in probe.columns:
            ...
            # Require some actual numeric county information.
        ...
        candidate_soil = probe[
            keep_cols
        ].copy()


        # ----------------------------------------------------
        # One static soil row per county.
        #
        # If the source has accidental duplicate county rows,
        # mean numeric aggregation is deterministic and does
        # not mix years because these are static soil features.
        # ----------------------------------------------------
        ...
        soil_static = candidate_soil

        SOC_FEATURES = soc_cols

        CEC_FEATURES = cec_cols

        SOC_CEC_FEATURES = sorted(
...
if soil_static is None:

    print(
        "No usable SOC/CEC SoilGrids table found."
    )

    print(
        "SOC/CEC variants will be skipped; the current model diagnostics "
        "will still run normally."
    )

    soil_static = pd.DataFrame(
        columns=[
            "FIPS"
        ]
    )
...
soil_static_indexed = (
    ...
    if "FIPS" in soil_static.columns
...
# ============================================================
# CELL 20 — DEFINE FEATURE GROUPS + ABLATION EXPERIMENTS
#
# DESIGN
# ------
# We use the SAME expanding-year validation as Cell 16:
#
#   train = all years < held-out year
#   test  = held-out year
#
# and the SAME XGBoost configuration.
#
# We run two complementary diagnostics:
#
# A) BUILD-UP LADDER
#    Separates raw vegetation from vegetation anomalies first,
#    then adds the remaining feature groups.
#
# B) DROP-ONE ABLATION
#    Starts from the current full model and removes one group,
#    including vegetation anomalies specifically.
#
# Why both?
# ---------
# Incremental build-up depends on ordering when predictors are correlated.
# Drop-one asks whether a group still contributes after all other current
# predictors are already present.
#
# NOTE ON "VEGETATION ONLY":
# --------------------------
# The model target remains yield_anomaly and absolute prediction is still:
#
#     predicted_yield = hist_5yr + predicted_anomaly
#
# Therefore "Vegetation only" means hist_5yr is NOT an X predictor.
# hist_5yr is still used only to reconstruct absolute yield, preserving the
# same target definition across every experiment.
# ============================================================

print()
print("=" * 105)
print("CELL 20 — FEATURE-GROUP ABLATION DESIGN")
print("=" * 105)


WEATHER_RAW_FEATURES = list(
    RAW_PRISM
)


WEATHER_ANOM_FEATURES = list(
    PRISM_ANOM_COLS
)
...
AWC_INTERACTION_FEATURES = [

    c

    for c in AWC_FEATURES
...
CURRENT_ENV_FEATURES = (

    WEATHER_RAW_FEATURES

    +

    WEATHER_ANOM_FEATURES
...
BUILD_UP_SPECS = [
    ...
    # --------------------------------------------------------
    # DIRECT TEST OF THE VEGETATION-ANOMALY FEATURE BLOCK
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # BUILD THE CURRENT MODEL FROM THE FULL VEGETATION BLOCK
    # --------------------------------------------------------
    ...
    _spec(
        "Veg + anomalies + hist + year + raw weather",
        veg_raw=True,
        veg_anom=True,
        hist=True,
        year=True,
...
# ------------------------------------------------------------
# SOC/CEC are NOT in the current production model.
# Add experimental variants only if the file was discovered.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Full season for build-up.
# Focus operational checkpoints for the expensive drop-one run.
# ------------------------------------------------------------
...
# ============================================================
# CELL 21 — FEATURE-GROUP MATRIX BUILDER
# ============================================================

print()
print("=" * 105)
print("CELL 21 — ABLATION MATRIX BUILDER")
print("=" * 105)


def _soil_matrix_for_rows(
    rows,
    columns
):
...
def _environment_columns_from_spec(
    spec
):
...
def build_ablation_matrices(
    ...
    # VEGETATION BASE
    #
    # build_features() from Cells 1-18 returns BOTH the raw
    # vegetation feature block and the historical vegetation-
    # anomaly feature block. We explicitly split them here.
    # --------------------------------------------------------
    ...
    # Engineered vegetation anomaly names are:
    #   NDVI_anom_mean
    #   EVI2_anom_last
    #   NIRv_anom_slope_last
    #
    # Raw vegetation features do not contain "_anom_".
    veg_anom_cols = [

        c

        for c in Xveg_train.columns
    ...
    Xveg_train = Xveg_train.reindex(
        columns=selected_veg_cols
    )


    Xveg_test = Xveg_test.reindex(
        columns=selected_veg_cols
    )
    ...
    env_cols = _environment_columns_from_spec(
        spec
    )
    ...
    # --------------------------------------------------------
    # HISTORICAL YIELD BASELINE AS AN X FEATURE
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # TRAINING-ONLY COLUMN FILTER + TRAINING-ONLY MEDIANS
    # --------------------------------------------------------
...
print(
    "✓ build_ablation_matrices() ready"
)


# ============================================================
# CELL 22 — RUN FEATURE ABLATION
# ============================================================

print()
print("=" * 105)
print("CELL 22 — RUNNING FEATURE-GROUP ABLATION")
print("=" * 105)
...
def _run_ablation_specs(
    ...
    for doy in doys:
        ...
        for test_year in ABLATION_TEST_YEARS:
            ...
            actual_yield = test_rows[
                "yield_bu_acre"
            ].to_numpy()


            reconstruction_baseline = test_rows[
                "hist_5yr"
            ].to_numpy()
            ...
            for spec in specs:
                ...
                X_train, X_test = build_ablation_matrices(
...
feature_ablation_results_df = pd.DataFrame(
    feature_ablation_results
)


if feature_ablation_results_df.empty:

    raise RuntimeError(
        "Feature ablation produced no results."
    )
...
# ============================================================
# CELL 23 — SUMMARIZE EFFECT SIZE
# ============================================================

print()
print("=" * 105)
print("CELL 23 — FEATURE ABLATION SUMMARY")
print("=" * 105)


feature_ablation_summary = (

    feature_ablation_results_df
...
# ------------------------------------------------------------
# BUILD-UP: paired ΔR² versus RAW vegetation only.
# ------------------------------------------------------------

build_results = feature_ablation_results_df[
...
build_results = build_results.merge(
...
build_gain_summary = (
    ...
    .agg(
        ...
        Mean_Delta_R2_vs_RawVegetation=(
            "Delta_R2_vs_RawVegetation",
            "mean"
        ),

        Mean_Delta_RMSE_vs_RawVegetation=(
            "Delta_RMSE_vs_RawVegetation",
            "mean"
        ),
...
# ------------------------------------------------------------
# DIRECT VEGETATION-ANOMALY TEST
#
...
#   Raw + vegetation anomalies
#
# Positive ΔR² means the historical vegetation anomaly feature
# block improves out-of-year yield prediction.
# ------------------------------------------------------------
...
vegetation_anomaly_gain = raw_plus_anom.merge(
...
vegetation_anomaly_gain[
    "Delta_R2_from_VegAnomalies"
] = (
...
vegetation_anomaly_gain_summary = (
    ...
    .agg(
        ...
        Mean_Delta_R2_from_VegAnomalies=(
            "Delta_R2_from_VegAnomalies",
            "mean"
        ),

        Mean_Delta_RMSE_from_VegAnomalies=(
            "Delta_RMSE_from_VegAnomalies",
            "mean"
        ),
...
# ------------------------------------------------------------
# DROP-ONE: paired loss/gain relative to current full model.
#
# Positive R2_Loss_When_Removed means removing the group hurts,
# therefore the group was helping.
# ------------------------------------------------------------

drop_results = feature_ablation_results_df[
...
drop_results = drop_results.merge(
...
drop_one_summary = (
    ...
    .agg(
        ...
        Mean_R2_Loss_When_Removed=(
            "R2_Loss_When_Removed",
            "mean"
        ),

        Mean_RMSE_Increase_When_Removed=(
            "RMSE_Increase_When_Removed",
            "mean"
        ),
...
# ------------------------------------------------------------
# Static soil contribution relative to the CURRENT model.
# ------------------------------------------------------------

soil_gain_rows = build_results[
...
if len(
    ...
    soil_gain_rows = soil_gain_rows.merge(
    ...
    soil_gain_summary = (
        ...
        .agg(
            ...
            Mean_Delta_R2_vs_Current=(
                "Delta_R2_vs_Current",
                "mean"
            ),

            Mean_Delta_RMSE_vs_Current=(
                "Delta_RMSE_vs_Current",
                "mean"
            ),
    ...
    soil_gain_summary = pd.DataFrame()


# ============================================================
# KEY CHECKPOINT TABLES
# ============================================================
...
print()
print(
    "MEAN R² — KEY CHECKPOINTS"
)
display(
    build_key_table.round(
        4
    )
)
...
print()
print(
    "ΔR² VS RAW VEGETATION-ONLY — KEY CHECKPOINTS"
)
display(
    build_key_gain_table.round(
        4
    )
)
...
print()
print(
    "DIRECT VEGETATION-ANOMALY GAIN"
)
print(
    "Positive ΔR² = vegetation anomalies improved prediction."
)
display(
...
print()
print(
    "DROP-ONE: R² LOSS WHEN FEATURE GROUP IS REMOVED"
)
print(
    "Positive = feature group was helping."
)
display(
    drop_key_table.round(
        4
    )
)


if not soil_gain_summary.empty:

    print()
    print(
        "EXPERIMENTAL SOC / CEC GAIN VS CURRENT MODEL"
    )
...
# ============================================================
# CELL 24 — SAVE TABLES + NON-BLOCKING FIGURES
# ============================================================

print()
print("=" * 105)
print("CELL 24 — SAVING FEATURE-DIAGNOSTIC OUTPUTS")
print("=" * 105)


FEATURE_DIAG_DIR = os.path.join(
...
paths_to_save = {
    ...
    "all_results":
        os.path.join(
            FEATURE_DIAG_DIR,
            "feature_ablation_all_heldout_results.csv"
        ),
    ...
    "build_gain":
        os.path.join(
            FEATURE_DIAG_DIR,
            "feature_build_up_gain_vs_raw_vegetation.csv"
        ),

    "veg_anomaly_gain":
        os.path.join(
            FEATURE_DIAG_DIR,
            "vegetation_anomaly_gain.csv"
        ),

    "drop_one":
        os.path.join(
            FEATURE_DIAG_DIR,
            "feature_drop_one_effect.csv"
        ),

    "soil_gain":
        os.path.join(
            FEATURE_DIAG_DIR,
            "feature_SOC_CEC_gain_vs_current.csv"
        )

}


feature_ablation_results_df.to_csv(
...
vegetation_anomaly_gain_summary.to_csv(
...
# ------------------------------------------------------------
# FIGURE 1 — vegetation vs current vs SOC+CEC extended
# ------------------------------------------------------------
...
plt.title(
    "Feature Ablation — Vegetation vs Current Environmental Model"
)
...
fig1_path = os.path.join(
    ...
    "feature_ablation_seasonal_R2.png"
...
# ------------------------------------------------------------
# FIGURE 2 — incremental gain over vegetation-only
# ------------------------------------------------------------
...
for variant in gain_plot_variants:
...
plt.ylabel(
    "Mean ΔR² vs raw vegetation-only"
)


plt.title(
    "Incremental Predictive Value Beyond Raw Vegetation"
)
...
fig2_path = os.path.join(
    ...
    "feature_build_up_delta_R2_vs_raw_vegetation.png"
...
# ------------------------------------------------------------
# FIGURE 3 — DIRECT VEGETATION-ANOMALY GAIN
# ------------------------------------------------------------
...
plt.ylabel(
    "Mean ΔR² from vegetation anomalies"
)


plt.title(
    "Value of Historical Vegetation-Anomaly Features"
)
...
veg_anom_fig_path = os.path.join(

    FEATURE_DIAG_DIR,

    "vegetation_anomaly_delta_R2.png"
...
# ------------------------------------------------------------
# FIGURE 4 — drop-one mean effect over operational checkpoints
# ------------------------------------------------------------

drop_plot = drop_one_summary[
...
if len(
    ...
    plt.ylabel(
        "Mean R² loss when removed"
    )


    plt.title(
        "Drop-One Feature Importance — Operational Checkpoints"
    )
...
print()
print(
    "CURRENT MODEL FEATURE BLOCKS:"
)


print(
    "  Raw vegetation features: YES"
)


print(
    "  Historical vegetation-anomaly features: YES"
)
...
print(
    "  SOC currently in production model: NO"
)


print(
    "  CEC currently in production model: NO"
)


print()
print(
    "Saved diagnostic directory:"
)
...
print()
print("=" * 105)
print("FEATURE-GROUP DIAGNOSTIC COMPLETE")
print("=" * 105)
